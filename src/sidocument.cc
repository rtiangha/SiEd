...
Position SiDocument::tidy_lines(const Position start_pos,const BlockInt byte_offset,Position & first_change_pos,Position & cursor_pos)
{
  ...
  while(p.line<get_number_blocks()&&changed&&(!m_memory_error_occurred))
      ...
      if( ( (!changed)||(has_done_insert())) && (!contains_line_feed) )
	{
	  ...
	  byte_start=p.x=0;
	  ++(p.line);
	  //check if the sloshing has created a word split over two lines
	  if(p.line<get_number_blocks())
	    {
	      check_new_broken_word(p,cursor_pos,changed,first_change_pos);
	      
	      //remember to set the wrapped flag so that the cursor position
	      //is correctly set
	    }
	}
      else
...
//! Ensure that there are no too long or too short lines
//! also returns the final position needing to be redrawn
/*Position SiDocument::tidy_lines(const Position start_pos,const BlockInt byte_offset,Position & first_change_pos,Position & cursor_pos)
{
  //starting at position start_pos, layout text onwards to be correct
  //also check previous line (start_pos.line-1)
  //and update cursor_pos if required when text is moved around
  #ifdef TEST_OBJECTS
  ErrFatalDisplayIf(byte_offset<start_pos.x,"Incorrect char count, tidy_lines");
  #endif
  if(start_pos.line>=get_number_blocks())
    return Position(0,0);
  ...
  //check that we don't need to pull text back to the previous line
  //  Boolean wrapped=wrap_back(start_pos,first_change_pos,cursor_pos,increase_redraw);
  ...
  block * curr_block;
  Boolean changed=true;
  Int16 b_b_index,curr_width=0;

  Position p=start_pos;

  Int16 first_wrap_line=NONE;

  Boolean first=true;
  //Go through each line, starting at the changed one
  //making sure each line is neither too long or too short
  //but just right. Stop when an unchanged line is found
  ...
  while(p.line<get_number_blocks() &&(changed)&&!m_memory_error_occurred)
    {
      ...
      if(byte_start==0)
	{
	  check_new_broken_word(p,cursor_pos,changed);
	  if(changed)
	    wrapped=true;
	}
      if(first)
	first=false;
      ...
      if(changed&&first)
	{
	  first_wrap_line=p.line; 
	}

      first=false;
      if(  (has_done_insert() ||
	    (!changed) )&& (!contains_line_feed))
	tidy_line_too_short(p.line,curr_width,changed,increase_redraw);
		
      byte_start=p.x=0;
      ++(p.line);
      //check if the sloshing has created a word split over two lines
      if(p.line<get_number_blocks()&&(!changed))
	{
	  check_new_broken_word(p,cursor_pos,changed);
  ...
  //set the earliest redraw position if there has been no
  //wrapping backwards
  if( (first_wrap_line!=SIGNED_NONE&&(!wrapped)) && (first_wrap_line<get_number_blocks()) )
    {
      first_change_pos.line=first_wrap_line;
      curr_block=get_line(first_wrap_line,b_b_index);
      first_change_pos.x=curr_block->n_chars-1;
      if(start_pos<first_change_pos)
	first_change_pos=start_pos;
      release_line(b_b_index);
    }

  //make sure no empty lines have been created at the end of the document
  //by all this sloshing. start on the last line and work backwards
  remove_empty_lines();

  --(p.line);
#ifdef TEST_OBJECTS_LOG
  log_entry("tidy lines done");
#endif
  if(get_number_blocks()>0)
    {
      if(increase_redraw)
	...
	p.line=get_number_blocks()-1;
      ...
      Int16 x=get_line(p.line,b_b_index)->n_chars;
      release_line(b_b_index);
      if(!wrapped)
	{
	  cursor_pos.line=NONE;
	}
      #ifdef TEST_OBJECTS
      log_entry_number("Tidy.x=",x);
      log_entry_number("Tidy.line=",p.line);
      #endif
      return Position(x,p.line);
    }
  else
    {
      cursor_pos.x=0;
	  ...
	  if(!(SiUtility::is_break_char(the_char)))
	    {
	      ...
	      block * prev_block=get_line(start_pos.line,b_b_index_prev);
	      if(prev_block->n_chars>0)
		{
		  ...
		  #endif
		  byte_offset=find_prev_delimiter(prev_block->mem_chunk->mem_ptr,byte_offset,char_offset);

		  if(char_offset>0&&char_offset<prev_block->n_chars-1)
		    {
		      BlockInt dist_to_move=prev_block->n_chars-char_offset;
		      release_line(b_b_index_prev);
		      release_line(b_b_index_start);

		      move_to_next_block(char_offset,byte_offset,start_pos.line);
		      
		      //move_position(cursor_pos,dist_to_move);

		      if(cursor_pos.x>char_offset)
			cursor_pos.x-=char_offset;
		      else
			cursor_pos.x+=dist_to_move;
		      cursor_pos.line=start_pos.line+1;
...
Boolean SiDocument::tidy_line_too_long(Position start_pos,BlockInt byte_offset,Int16 & curr_width,Boolean & changed,Boolean * increase_redraw)
{
  ...
  if(curr_block->n_chars==0)
	...
	{
	  last_line_tidied=start_pos.line;
	}
      else
	{
	  curr_width=0;
  ...
  if(start_pos.line>last_line_tidied)
    last_line_tidied=start_pos.line;
  //Boolean contains_line_feed=false;
  WChar the_char;
  changed=false;
      ...
      if(the_char==chrLineFeed)
	{
	  ...
	  if(start_pos.x<(curr_block->n_chars))
		...
		(*increase_redraw)=true;
	      ...
	      //improve the line width estimate for incremental loaind if necessary
...
#endif
	      byte_offset=find_prev_delimiter(curr_block->mem_chunk->mem_ptr,byte_offset,start_pos.x);
      ...
      if(number_to_move<next_block->n_chars&&!stop)
	{
	  if (prev_delim_char>0)
	  ...
	  else
	    {
	      /*if(curr_block->n_chars>0)
		{
		  the_char=get_char(line,curr_block,curr_block->n_chars-1);
		  if (SiUtility::is_break_char(the_char))
...
BlockInt SiDocument::find_prev_delimiter(const Char * data,const BlockInt byte_offset,BlockInt& char_offset)
{
  ...
  while(c_offset>1)
    {
      ...
      if(TxtCharIsSpace(the_char))
	{
	  if(c_offset==char_offset-1)
	    {
	      --char_offset;
	      #ifdef TEST_OBJECTS
	      ErrFatalDisplayIf(b_offset<char_offset,"Incorrect char_offset, find_prev_delimiter 1");
	      #endif
	      return b_offset;
	    }
	  else
...
#ifdef TEST_OBJECTS
	      ErrFatalDisplayIf(b_offset+TxtCharSize(the_char)<char_offset,"Incorrect char_offset, find_prev_delimiter 2");
	      #endif
	      return b_offset+TxtCharSize(the_char);
	    }
	}
    }
  ...
  if(byte_offset==0)
    {
      #ifdef TEST_OBJECTS
      ErrFatalDisplayIf(char_offset!=0,"Incorrect char_offset,find_prev_delimiter");
      #endif
      return 0;
    }
  else
    {
      return byte_offset-TxtPreviousCharSize(data,byte_offset); 
    }
}

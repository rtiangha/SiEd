...
class SiMemHandler
{
 public:
  ...
  static void 		initialise_memory() UTIL_SECTION;
  static void 		exit_memory() UTIL_SECTION;
  static SiMemChunk * 	get_chunk(SiMemChunk* parent,const BlockInt size,const Int16 allocation_type) UTIL_SECTION ;
  static SiMemChunk * 	get_chunk_quick(const BlockInt size)  UTIL_SECTION;
  static SiMemChunk *	resize_chunk(SiMemChunk * chunk,const BlockInt new_size,const UInt8 direction) UTIL_SECTION;
  static void 		free_chunk(SiMemChunk * chunk)  UTIL_SECTION;
  static SiMemChunk *	move_data(SiMemChunk * src_chunk,SiMemChunk * dst_chunk,const BlockInt src_offset,const UInt8 direction) ;  
  static void 			compact_memory() UTIL_SECTION;

#ifdef TEST_OBJECTS
  static void mem_show_mem_usage() UTIL_SECTION;
  ...
  static void perform_tests() UTIL_SECTION;
#endif
#ifdef TEST_OBJECTS
  static UInt32 mem_get_free_chunk_count() UTIL_SECTION;
  static UInt32 mem_get_allocated_chunk_count() UTIL_SECTION;
  static void output_chunk_list() UTIL_SECTION;
  
#endif
  static UInt16 MEMORY_CHUNK_SIZE;
 private:
#ifdef TEST_OBJECTS
  static void check_heap_consistency() UTIL_SECTION;
#endif

  static void defragment_chunks(SiMemChunk * start_chunk) UTIL_SECTION;
  static SiMemChunk * split_chunk(SiMemChunk * chunk,const BlockInt split_pos,const UInt8 direction) UTIL_SECTION;
  static SiMemChunk* reallocate_chunk(SiMemChunk * chunk,const BlockInt new_size,const UInt8 direction) UTIL_SECTION;
  ...
  static SiMemChunk * check_for_zero_size(SiMemChunk * chunk) UTIL_SECTION;

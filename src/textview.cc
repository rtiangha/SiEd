...
void SiTextView::clear_screen()
{
  ...
  /*	rp.topLeft.x = 0;
	...
	WinEraseRectangle(&screen_size, 0);
	cursor_drawn = false;
}
...
void SiTextView::get_cursor_window()
{
  ...
  clear_rect.extent.y = FntLineHeight();
  ...
  cursor_backing_store =
    WinCreateOffscreenWindow(CURSOR_WIDTH*2,line_height*2, nativeFormat,&err);
...
void SiTextView::notify_text_changed(Position start,Position end)
{
...
#endif
#ifdef TEST_OBJECTS
	ErrFatalDisplayIf(start.line<0||end.line<0,"drawing off start of document");
	//	Char buff[30];
	
	//StrPrintF(buff,"Delete start=%i",start.x);
	...
	WinResetClip();
	#ifdef LOG_ENTRY
	log_entry("notify_text_changed done");
	#endif
}
...
void SiTextView::clear_after_line(const BlockInt line,Boolean tidy_end)
{
  if(!update_screen_enabled)
	...
	if(cursor_location.line>=clear_rect.topLeft.y)
	  {
	    cursor_drawn=false;	   
	  }
...
void SiTextView::erase_cursor()
{
	...
	topLeft_destx = cursor_location.x -CURSOR_X_OFFSET;
	topLeft_desty = get_y(cursor_location.line);
	...
	WinCopyRectangle(cursor_backing_store, NULL, &src_rect, topLeft_destx,
...
void SiTextView::redraw_cursor()
{
	...
	Int16 top_y=get_y(cursor_location.line);
	Int16 bottom_y=top_y+line_height-2;
	if(top_y>=m_editor->TOP_Y&&bottom_y<=m_editor->BOTTOM_Y)
	{
		WinPushDrawState();
		if(SiUtility::has_colour_support())
		  WinSetForeColor(SiUtility::get_cursor_color());
		WinPaintLine(cursor_location.x -CURSOR_X_OFFSET,top_y,cursor_location.x -CURSOR_X_OFFSET,bottom_y);
		cursor_drawn = true;
		WinPopDrawState();
	}
}
...
void SiTextView::draw_cursor(DrawPosition p)
{
	...
	cursor_location.x = p.x;
	cursor_location.line=(p.line-m_editor->TOP_Y)/line_height+top_line;
	Coord topLeft_destx, topLeft_desty;
	topLeft_destx = 0;
	topLeft_desty = 0;
	RectangleType src_rect;
	src_rect.topLeft.x = cursor_location.x-CURSOR_X_OFFSET;
	src_rect.topLeft.y = p.line;
	src_rect.extent.x = CURSOR_WIDTH;
	src_rect.extent.y = line_height;

	//copy the current background of the cursor
	WinCopyRectangle(NULL, cursor_backing_store, &src_rect, topLeft_destx,topLeft_desty, winPaint);
...
void SiTextView::draw_status_filename()
{
	...
	if(SiUtility::has_colour_support())
	  ...
	  WinDrawTruncChars(f, StrLen(f), BORDER,m_editor->HEIGHT+m_editor->TOP_Y-STATUS_BAR_HEIGHT-1+d_height, FILENAME_WIDTH);
	
	if(SiUtility::has_colour_support())
	  WinSetTextColor(prev_color);

	next_width = FntCharsWidth(f, StrLen(f)) + BORDER;
...
void SiTextView::draw_status_line_number()
{
	...
	if(text_buff!=NULL)
	{
	  fontID prev_font=FntSetFont(stdFont);
		...
		status_cursor_line = cursor_location.line;
		status_last_line=m_document->get_apparent_number_blocks();
		
		StrPrintF(text, "L%i/%i", cursor_location.line+1,m_document->get_apparent_number_blocks());
		...
		WinEraseLine(LINE_DISPLAY_POS+prev_dis_width+1,m_editor->HEIGHT+m_editor->TOP_Y-2,LINE_DISPLAY_POS+prev_dis_width+1,m_editor->HEIGHT+m_editor->TOP_Y-STATUS_BAR_HEIGHT-1);
		...
		WinDrawLine(LINE_DISPLAY_POS+width+1,m_editor->HEIGHT+m_editor->TOP_Y-1,LINE_DISPLAY_POS+width+1,m_editor->HEIGHT+m_editor->TOP_Y-STATUS_BAR_HEIGHT-2);
		if(SiUtility::has_colour_support())
		  WinSetForeColor(prev_color);
...
void SiTextView::draw_status_dis(Boolean force)
{
	...
	if (status_cursor_line != cursor_location.line || force||status_last_line!=m_document->get_number_blocks()||force)
...
Boolean SiTextView::point_in_drag_area(const Int16 x,const Int16 y)
{
	...
	if(y>=get_status_y()&&y<=m_editor->HEIGHT+m_editor->TOP_Y)
	{
		...
		if(x>=LINE_DISPLAY_POS+prev_dis_width+1&&x<=SiUtility::SCREEN_WIDTH)
		{
...
Int16 SiTextView::point_in_line_number(Coord x,Coord y)
{
  ...
  if(x>=LINE_DISPLAY_POS&&x<=LINE_DISPLAY_POS+prev_dis_width)
      ...
      if(y<=m_editor->HEIGHT+m_editor->TOP_Y-1&&y>=m_editor->HEIGHT+m_editor->TOP_Y-STATUS_BAR_HEIGHT-2)
	{
	  FormPtr frm=FrmGetFormPtr(ResformID_text);
	  ListType * lst;
	  if(m_editor->TOP_Y==0&&m_editor->HEIGHT<SiUtility::SCREEN_HEIGHT)
	    {
